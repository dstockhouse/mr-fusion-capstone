...
foreach($map_xml->Document->Placemark as $pm) {
    ...
    if($pm->Point) {
        ...
        $json_coords = array(
                        // lon2y(floatval($coords[0])),
                        // lat2x(floatval($coords[1])));
                        floatval($coords[0]),
                        floatval($coords[1]));
        ...
        foreach($coords_strings as $coord) {
            if($coord) {
                ...
                array_push($json_coords, array(
                            // lon2y(floatval($current[0])),
                            // lat2x(floatval($current[1]))));
                            floatval($current[0]),
                            floatval($current[1])));
            }
        }
...
$path_json = $map_graph->find_json_path(
        // array(34.6147979, -112.4509615),
        // array(34.6159122, -112.4489055));
        array(-112.4509615, 34.6147979),
        array(-112.4489055, 34.6159122));
