 ...
 *
 * Description:
 *	Driver for Kangaroo x2 motion controller
 * 
 * Author:
 * 	David Stockhouse
 *
 ...
 *
 * Initializes a Kangaroo motion controller device object
 *
 * Arguments: 
 *     dev     - Pointer to KANGAROO_DEV instance to initialize
 *     devname - Pointer to name of device file
 *     baud    - Baud rate to initialize device
 *
 * Return value:
 *     On success, returns 0
 *     On failure, returns a negative number
...
int KangarooInit(KANGAROO_DEV *dev, char *devname, int baud) {
    ...
    if (devname == NULL) {
        dev->fd = UARTInit(KANGAROO_DEVNAME, baud);
    } else {
        dev->fd = UARTInit(devname, baud);
    }

    if (dev->fd < 0) {
        logDebug(L_INFO, "Couldn't initialize Kangaroo motion controller\n");
        return -2;
    }
