...
switch direction
    ...
    case 6
        ...
        x = zeros(1,6/time_step);
        y1 = -1+time_step:time_step:0;
        y2 = 0-time_step:-time_step*0.86:-5;
        y = cat(2,y1,y2);
...
%--------------------------------------------------------------------------
% Print simulation info in figure
%--------------------------------------------------------------------------
dist = sqrt((K(1,1)-x(increment))^2 + (K(2,1)-y(increment))^2);

str1 = ['Heading Change: ' num2str(abs(rad2deg(heading)), '%.2f') ' deg'];
